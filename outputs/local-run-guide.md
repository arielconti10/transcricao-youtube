...
O site foi executado localmente em 17 de julho de 2026 e produziu uma
...
- Resultado: 166 palavras e 903 caracteres
- Resposta cortada: não
- Endpoint local: `200 OK`
- Erros no navegador: nenhum
- Verificação automatizada: 38 testes aprovados
...
```text
http://127.0.0.1:4173/#SEU_FAMILY_ACCESS_TOKEN
```

Substitua `SEU_FAMILY_ACCESS_TOKEN` pelo valor que já está no ficheiro `.env`.
Não coloque a chave da Gemini no endereço.
...
Configure `GEMINI_API_KEY` e `FAMILY_ACCESS_TOKEN` como variáveis secretas no
